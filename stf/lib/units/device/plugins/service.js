      ...
      if (envelope.id !== null) {
        ...
        switch (envelope.type) {
          ...
          case apk.wire.MessageType.EVENT_BATTERY:
            ...
            break
          case apk.wire.MessageType.EVENT_CPU:
            message = apk.wire.CpuEvent.decode(envelope.message)
            push.send([
              wireutil.global
              , wireutil.envelope(new wire.CpuEvent(
                options.serial
                , message.cpu_consumed
                , message.mem_shared
                , message.mem_standalone
              ))
            ])
            plugin.emit('CpuEvent', message)
            break
